...
class OllamaLLM(BaseLLM):
    ...
    def _iter_tokens(self, prompt: str, system_prompt: str = None):
        ...
        # Only send num_ctx if the user explicitly sets it — letting
        # Ollama pick the default keeps per-token latency low on CPU.
        _num_ctx = os.environ.get("OLLAMA_NUM_CTX")
        if _num_ctx:
